import img from "../assets/header.png";

const Header = () => {
  return (
    <div
      style={{
        background: `linear-gradient(to bottom, rgb(8 51 68), rgba(240, 240, 240, .5)), url(${img})`,
      }}
      className="w-full"
    >
      <div className="w-[95%] md:w-[80%] mx-auto flex md:flex-row flex-col-reverse text-center md:text-left items-center justify-between">
        <div className="W-full space-y-4 md:w-1/2 py-10 md:py-32">
          <h1 className=" text-2xl md:text-4xl text-[#ffffffe0] font-bold">
            Explore Next-Level Technology and Electronics.
          </h1>
          <p className="text-[#ffffff8c]">
            Welcome to a world of innovation and quality. Discover a wide range
            of technology and electronics devices proudly presented by Apple,
            Samsung, Sony, Google, Intel, Oppo. From smart gadgets to
            high-performance electronics, we are here to make your digital
            dreams a reality. Shop now and experience the future of tech with
            us.
          </p>
        </div>
        <div className="relative grid grid-cols-2 gap-3 rounded-full md:w-80 my-10  h-80">
          <div className="md:absolute top-36 -left-10">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              x="0px"
              y="0px"
              width="100"
              height="100"
              viewBox="0 0 48 48"
            >
              <path
                fill="#fbc02d"
                d="M43.611,20.083H42V20H24v8h11.303c-1.649,4.657-6.08,8-11.303,8c-6.627,0-12-5.373-12-12	s5.373-12,12-12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C12.955,4,4,12.955,4,24s8.955,20,20,20	s20-8.955,20-20C44,22.659,43.862,21.35,43.611,20.083z"
              ></path>
              <path
                fill="#e53935"
                d="M6.306,14.691l6.571,4.819C14.655,15.108,18.961,12,24,12c3.059,0,5.842,1.154,7.961,3.039	l5.657-5.657C34.046,6.053,29.268,4,24,4C16.318,4,9.656,8.337,6.306,14.691z"
              ></path>
              <path
                fill="#4caf50"
                d="M24,44c5.166,0,9.86-1.977,13.409-5.192l-6.19-5.238C29.211,35.091,26.715,36,24,36	c-5.202,0-9.619-3.317-11.283-7.946l-6.522,5.025C9.505,39.556,16.227,44,24,44z"
              ></path>
              <path
                fill="#1565c0"
                d="M43.611,20.083L43.595,20L42,20H24v8h11.303c-0.792,2.237-2.231,4.166-4.087,5.571	c0.001-0.001,0.002-0.001,0.003-0.002l6.19,5.238C36.971,39.205,44,34,44,24C44,22.659,43.862,21.35,43.611,20.083z"
              ></path>
            </svg>
          </div>
          <div className="md:absolute -top-5 left-5">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              x="0px"
              y="0px"
              width="100"
              height="100"
              viewBox="0 0 48 48"
            >
              <path
                fill="#0883d9"
                d="M36.232,23.985c0-5.865,4.766-8.51,4.966-8.636c-2.596-3.993-6.79-4.467-8.362-4.468	c-3.643,0-6.863,2.022-8.585,2.022c-1.797,0-4.418-2.121-7.363-2.022c-3.843,0.075-7.363,2.346-9.334,5.691	c-1.397,2.396-1.947,5.217-1.896,8.087c0.002,0.113,0.017,0.228,0.02,0.341H36.32C36.279,24.671,36.243,24.337,36.232,23.985z"
              ></path>
              <path
                fill="#0883d9"
                d="M30.565,7.063C32.261,5.191,33.21,2.621,33.06,0c-2.346,0-5.066,1.372-6.788,3.394	c-1.348,1.672-2.795,4.293-2.271,6.913C26.422,10.607,29.043,9.085,30.565,7.063z"
              ></path>
              <path
                fill="#0370c8"
                d="M17.511,45c2.771,0,3.794-1.848,7.413-1.848c3.37,0,4.418,1.848,7.338,1.848	c3.07,0,5.092-2.795,6.913-5.567c2.295-3.218,3.07-6.288,3.169-6.414c-0.094,0-5.287-2.112-6.026-8.019H5.678	c0.157,5.311,2.228,10.79,4.671,14.309C12.27,42.055,14.441,45,17.511,45z"
              ></path>
            </svg>
          </div>
          <div className="md:absolute -top-5 right-4">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              x="0px"
              y="0px"
              width="100"
              height="100"
              viewBox="0 0 48 48"
            >
              <path
                fill="#0d47a1"
                d="M47.97,19.826c0.654,3.747-9.547,8.655-22.788,10.96c-13.238,2.306-24.5,1.136-25.153-2.613 c-0.653-3.747,9.551-8.654,22.79-10.958C36.059,14.907,47.318,16.078,47.97,19.826z"
              ></path>
              <polygon
                fill="#fafafa"
                points="35.51,25.388 35.442,21.492 36.671,21.492 36.671,26.403 34.905,26.403 33.678,22.373 33.652,22.373 33.72,26.403 32.499,26.403 32.499,21.492 34.342,21.492 35.483,25.388"
              ></polygon>
              <polygon
                fill="#fafafa"
                points="13.177,21.952 12.497,26.455 11.157,26.455 12.076,21.492 14.284,21.492 15.201,26.455 13.865,26.455 13.204,21.952"
              ></polygon>
              <polygon
                fill="#fafafa"
                points="18.964,25.286 19.577,21.492 21.601,21.492 21.708,26.455 20.468,26.455 20.435,21.993 20.409,21.993 19.58,26.455 18.321,26.455 17.49,21.993 17.464,21.993 17.433,26.455 16.19,26.455 16.3,21.492 18.325,21.492 18.937,25.286"
              ></polygon>
              <path
                fill="#fafafa"
                d="M9.067,25.055c0.049,0.12,0.034,0.275,0.011,0.368c-0.042,0.165-0.154,0.333-0.483,0.333 c-0.312,0-0.5-0.179-0.5-0.45v-0.48H6.763L6.762,25.21c0,1.106,0.871,1.441,1.805,1.441c0.898,0,1.637-0.307,1.754-1.134 c0.061-0.429,0.015-0.709-0.005-0.816c-0.209-1.039-2.093-1.349-2.233-1.93c-0.024-0.099-0.017-0.205-0.005-0.262 c0.035-0.158,0.143-0.332,0.453-0.332c0.29,0,0.461,0.18,0.461,0.45c0,0.091,0,0.307,0,0.307h1.237v-0.348 c0-1.081-0.97-1.25-1.673-1.25c-0.883,0-1.604,0.292-1.736,1.099c-0.036,0.223-0.041,0.422,0.011,0.671 C7.049,24.118,8.811,24.412,9.067,25.055z"
              ></path>
              <path
                fill="#fafafa"
                d="M25.204,25.046c0.049,0.119,0.033,0.27,0.011,0.363c-0.041,0.165-0.152,0.33-0.479,0.33 c-0.307,0-0.494-0.179-0.494-0.444l-0.001-0.476h-1.318l-0.002,0.379c0,1.095,0.863,1.426,1.787,1.426 c0.888,0,1.62-0.303,1.736-1.122c0.061-0.426,0.018-0.702-0.004-0.807c-0.208-1.029-2.073-1.336-2.211-1.912 c-0.024-0.099-0.017-0.203-0.005-0.257c0.036-0.16,0.142-0.329,0.449-0.329c0.288,0,0.455,0.175,0.455,0.444 c0,0.09,0,0.304,0,0.304h1.228v-0.345c0-1.07-0.962-1.237-1.659-1.237c-0.873,0-1.588,0.288-1.717,1.09 c-0.036,0.22-0.04,0.415,0.012,0.663C23.206,24.118,24.951,24.41,25.204,25.046z"
              ></path>
              <path
                fill="#fafafa"
                d="M29.372,25.713c0.344,0,0.451-0.238,0.475-0.36c0.01-0.054,0.013-0.125,0.012-0.19V21.49h1.255 v3.56c0.003,0.091-0.006,0.279-0.011,0.325c-0.088,0.927-0.821,1.227-1.732,1.227c-0.913,0-1.646-0.301-1.733-1.227 c-0.004-0.047-0.013-0.235-0.011-0.325v-3.56h1.254v3.672c0,0.064,0.002,0.137,0.012,0.19 C28.921,25.473,29.025,25.713,29.372,25.713z"
              ></path>
              <path
                fill="#fafafa"
                d="M39.725,25.66c0.359,0,0.485-0.227,0.508-0.359c0.009-0.057,0.012-0.126,0.011-0.189v-0.72 h-0.509v-0.724h1.76V25c-0.001,0.093-0.003,0.162-0.018,0.327c-0.082,0.903-0.866,1.225-1.745,1.225 c-0.881,0-1.663-0.322-1.747-1.225c-0.014-0.166-0.016-0.234-0.018-0.327l0.001-2.089c0-0.088,0.011-0.244,0.021-0.327 c0.11-0.928,0.862-1.226,1.743-1.226c0.88,0,1.651,0.297,1.742,1.226c0.016,0.158,0.011,0.327,0.011,0.327v0.166h-1.251v-0.278 c0.001,0.001-0.002-0.118-0.016-0.189c-0.021-0.11-0.116-0.362-0.495-0.362c-0.362,0-0.467,0.238-0.494,0.362 c-0.015,0.065-0.021,0.154-0.021,0.234v2.27c-0.001,0.063,0.003,0.132,0.013,0.189C39.241,25.433,39.366,25.66,39.725,25.66z"
              ></path>
            </svg>
          </div>
          <div className="md:absolute top-24 -right-10">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="100"
              height="100"
              viewBox="0 0 569.579 375.614"
              id="intel"
            >
              <path
                fill="#0e7dc2"
                d="M270.558 334.834c-112.551 10.383-229.861-5.971-246.292-93.93-8.038-43.361 11.742-89.341 37.946-117.92v-15.261c-47.204 41.399-72.786 93.782-58.051 155.661 18.822 79.367 119.477 124.312 273.155 109.387 60.829-5.895 140.381-25.453 195.724-55.852v-43.152c-50.228 29.927-133.287 54.648-202.482 61.067zm296.497-228.142c-26.697-130.341-279.771-138.604-442.907-39.3V78.36C287.097-5.414 518.232-4.905 539.276 115.163c7.062 39.718-15.312 81.097-55.282 104.923v31.154c48.081-17.636 97.4-74.678 83.061-144.548z"
              ></path>
              <path
                fill="#0e7dc2"
                d="M463.94 87.712h-29.51v131.432c0 15.403 7.446 28.798 29.51 30.938V87.712zM112.561 135.7H82.997l-.016 85.869c0 15.457 7.47 28.838 29.58 30.938V135.7zM82.997 91.71h29.472v27.973H82.997zM289.291 251.147c-23.886 0-33.994-16.602-33.994-32.955V104.178h29.253V135.7h22.031v23.628H284.55v57.057c0 6.668 3.166 10.383 10.119 10.383h11.912v24.38l-17.29-.001M343.12 202.573c0 14.971 9.361 25.973 25.925 25.973 12.979 0 19.376-3.602 26.868-11.002l18.076 17.219c-11.589 11.449-23.731 18.41-45.16 18.41-27.995 0-54.818-15.279-54.818-59.823 0-38.081 23.438-59.607 54.169-59.607 31.24 0 49.193 25.2 49.193 58.279v10.554H343.12m23.778-44.639c-10.013 0-17.706 5.157-20.967 12.113-1.899 4.181-2.595 7.439-2.812 12.577h45.084c-.57-12.577-6.272-24.69-21.305-24.69zM192.013 159.328c8.578 0 12.144 4.241 12.144 11.124v80.866h29.26v-81.036c0-16.42-8.749-34.551-34.443-34.551l-60.441-.031v115.618h29.271v-91.99h24.209zM495.691 111.197c-6.505 0-11.697-5.246-11.697-11.743 0-6.466 5.192-11.742 11.697-11.742 6.473 0 11.788 5.276 11.788 11.742 0 6.497-5.316 11.743-11.788 11.743zm0-21.527c-5.361 0-9.673 4.431-9.673 9.784 0 5.384 4.312 9.737 9.673 9.737 5.36 0 9.811-4.353 9.811-9.737-.001-5.353-4.45-9.784-9.811-9.784zm5.206 16.822h-2.194a.495.495 0 0 1-.446-.293l-2.968-5.045c-.093-.108-.34-.231-.416-.231h-1.359v5.029c0 .248-.171.541-.494.541h-1.979c-.295 0-.494-.293-.494-.541V93.267c0-.742.247-1.047.896-1.14.711-.077 2.565-.17 3.602-.17 3.661 0 5.81 1.078 5.81 4.473v.248c0 2.097-1.035 3.24-2.642 3.75l3.09 5.261a.945.945 0 0 1 .077.372c-.004.199-.111.431-.483.431zm-2.842-10.062c0-1.417-.881-1.865-2.767-1.865h-1.869v4.024c.294 0 1.623.062 1.869.062 1.886 0 2.767-.629 2.767-1.973v-.248z"
              ></path>
            </svg>
          </div>
          <div className="md:absolute bottom-3 -right-6">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 40 40"
              id="sony"
              width="100"
              height="100"
            >
              <path d="M10.22 19.21a2.91 2.91 0 0 0-.59-.21A16.55 16.55 0 0 0 8 18.75a9.51 9.51 0 0 1-1.47-.2.34.34 0 0 1-.27-.32.47.47 0 0 1 .25-.39 2.94 2.94 0 0 1 1.35-.27 3.86 3.86 0 0 1 1.56.31 1.86 1.86 0 0 1 .38.22 1.13 1.13 0 0 1 .36.61h.59v-1.6h-.66v.18c0 .06-.06.14-.18.08a4.9 4.9 0 0 0-2.16-.49 4.54 4.54 0 0 0-1.8.35A1.36 1.36 0 0 0 5 18.46a1.25 1.25 0 0 0 .34.87 2 2 0 0 0 1 .52c.29.06.89.16 1.5.22s1.19.12 1.43.18.51.12.51.48a.53.53 0 0 1-.4.49 3 3 0 0 1-1.19.21 4.85 4.85 0 0 1-1.42-.25 1.61 1.61 0 0 1-.77-.55.91.91 0 0 1-.17-.52h-.68V22h.73v-.25a.11.11 0 0 1 .12-.13 5.71 5.71 0 0 0 1.11.38 4.61 4.61 0 0 0 1.11.14 4.88 4.88 0 0 0 1.5-.21 2.59 2.59 0 0 0 .88-.44 1.21 1.21 0 0 0 .48-1 1.36 1.36 0 0 0-.39-1 1.51 1.51 0 0 0-.47-.28zm8.24-1.41a3.92 3.92 0 0 0-2.77-.93 4.34 4.34 0 0 0-2.77.93 2.29 2.29 0 0 0-.75 1.71 2.24 2.24 0 0 0 .75 1.71 4.12 4.12 0 0 0 2.77.93 4.08 4.08 0 0 0 2.77-.93 2.36 2.36 0 0 0 .75-1.71 2.31 2.31 0 0 0-.75-1.71zm-1.29 3.09a2.09 2.09 0 0 1-1.48.57 2.16 2.16 0 0 1-1.49-.57 1.94 1.94 0 0 1-.54-1.38 1.9 1.9 0 0 1 .54-1.38 2.11 2.11 0 0 1 1.49-.57 2.09 2.09 0 0 1 1.48.57 1.91 1.91 0 0 1 .55 1.38 1.87 1.87 0 0 1-.55 1.38zm7.32-3.14h.61c.28 0 .32.11.33.37v2l-3.23-2.91h-2.3v.58h.51c.37 0 .4.21.4.39v2.75c0 .16 0 .39-.23.39H20v.58h2.71v-.58H22c-.26 0-.27-.12-.28-.38v-2.39l3.72 3.31h.94v-3.74c0-.28 0-.37.31-.37h.59v-.58h-2.79zm7.86-.58v.58h.4a.12.12 0 0 1 .1.19l-1.13 1.31-1.1-1.32a.1.1 0 0 1 .07-.18h.39v-.58h-3.26v.58h.57a.7.7 0 0 1 .51.26c.08.09 1.72 1.95 1.76 2s0 .06 0 .08V21a.49.49 0 0 1 0 .12.2.2 0 0 1-.13.12h-.73v.58H33v-.58h-.76a.2.2 0 0 1-.13-.12.53.53 0 0 1 0-.12v-.9a.16.16 0 0 1 0-.09l1.78-2a.51.51 0 0 1 .47-.23H35v-.58z"></path>
            </svg>
          </div>
          <div className="md:absolute -bottom-10 left-24">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="100"
              height="100"
              fill="none"
              viewBox="0 0 48 48"
              id="mi"
            >
              <path
                fill="#FF6700"
                d="M0 24C0 10.7452 10.7452 0 24 0C37.2548 0 48 10.7452 48 24C48 37.2548 37.2548 48 24 48C10.7452 48 0 37.2548 0 24Z"
              ></path>
              <path
                fill="#fff"
                fillRule="evenodd"
                d="M24.6844 16H11.4691C11.3202 16 11.2 16.1176 11.2 16.2612V31.9086C11.2 32.0522 11.3202 32.1698 11.4691 32.1698H14.9381C15.087 32.1698 15.2072 32.0522 15.2072 31.9086V19.6284C15.2072 19.4873 15.3273 19.3698 15.4762 19.3698H22.9525C25.0553 19.3698 25.6509 20.9607 25.6509 21.9873V31.9086C25.6509 32.0522 25.7711 32.1698 25.92 32.1698H29.3864C29.5353 32.1698 29.6555 32.0522 29.6555 31.9086V20.8091C29.6555 19.8922 29.5432 18.5757 28.3494 17.4184C27.1007 16.209 25.9592 16 24.6844 16ZM36.5309 16H33.0619C32.913 16 32.7928 16.1176 32.7928 16.2612V31.9086C32.7928 32.0522 32.913 32.1698 33.0619 32.1698H36.5309C36.6798 32.1698 36.8 32.0522 36.8 31.9086V16.2612C36.8 16.1176 36.6772 16 36.5309 16ZM18.6109 22.2328H22.255C22.4039 22.2328 22.5241 22.3504 22.5241 22.4914V31.9112C22.5241 32.0549 22.4013 32.1724 22.255 32.1724H18.6109C18.4647 32.1724 18.3419 32.0549 18.3419 31.9112V22.494C18.3419 22.3504 18.462 22.2328 18.6109 22.2328Z"
                clipRule="evenodd"
              ></path>
            </svg>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Header;
